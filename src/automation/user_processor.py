...
class UserProcessor:
    ...
    async def _ensure_modal_closed(self):
        ...
        try:
            ...
            if await modal.count() > 0 and await modal.is_visible():
                ...
                try:
                    await self.page.wait_for_selector(self.SELECTORS["modal"], state="hidden", timeout=2000)
                except:
    ...
    async def _process_via_details(self, row: Locator) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Navigating to details...")
            ...
            # Find Planned row
            planned_row = table.locator('tbody tr').filter(has=self.page.locator(self.SELECTORS["status_planned"])).first
